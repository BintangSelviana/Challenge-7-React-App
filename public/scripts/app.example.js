  ...
  run = () => {
    ...
    Car.list.forEach((car) => {
      ...
      console.log(car.availableAt)
        ...
        let waktue = Date.parse(car.availableAt)

      if (car.available == driveri && waktue >= formdate && car.capacity >= penumpang){
        const node = document.createElement("div");
        node.className = "card m-5";
        node.className = "product";
        node.className = "cardd";
        // node.className = "container cardd";
